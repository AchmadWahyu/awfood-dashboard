...
import {
  getClosings, getClosingById, getItems, getSuppliers, addRequestEdit, getRequestEdits,
} from "@/lib/dummy/api";
import type { ClosingItem, RequestEdit } from "@/lib/dummy/types";
...
function EmployeeRequestEditPage() {
  const [version, setVersion] = useState(0);
  useSyncStorage(() => setVersion((v) => v + 1));
  const { user } = useAuth();
  const searchParams = useSearchParams();

  const myClosings = useMemo(
    () => getClosings().filter((c) => c.staff_id === user?.id && c.status !== "draft"),
    [user, version]
  );
  ...
  const selectedClosing = useMemo(() => (closingId ? getClosingById(closingId) : null), [closingId, version]);

  const loadClosing = (id: string) => {
    setClosingId(id);
    const c = getClosingById(id);
    if (!c) return;
        ...
        const entries = c.items
          .map((ci) => {
            ...
            return def && def.supplier_id === s.id
              ? { productId: def.id, productName: def.name, hargaJual: def.price_sell, stokAwal: ci.stok_awal, stokAkhir: ci.stok_akhir }
              : null;
          })
  ...
  useEffect(() => {
    ...
    if (id && myClosings.some((c) => c.id === id)) {
      loadClosing(id);
    }
  }, [searchParams, myClosings]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addRequestEdit({
      ...
      approved_at: null,
      items: allItems,
      cash_initial: selectedClosing.cash_initial,
  ...
  const myReqs = useMemo(
    () => getRequestEdits().filter((r) => r.requested_by === user?.id).sort((a, b) => +new Date(b.requested_at) - +new Date(a.requested_at)),
    [user, version]
  );
      ...
      <div className=" rounded-2xl border border-notch-border bg-paper-light p-6 shadow-sm space-y-4">
        <div>
          ...
          >
            ...
            {myClosings.map((c) => <option key={c.id} value={c.id}>{c.date} — {c.staff_name}</option>)}
          </select>
        </div>
        ...
        {selectedClosing && !ok && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl bg-marker-light/50 border border-marker/20 px-4 py-3">
              ...
              <button type="submit" className="w-full sm:w-auto rounded-xl bg-marker px-5 py-2.5 text-sm font-bold text-white hover:bg-marker-hover transition-colors">
                Ajukan Request Edit
              </button>
            </div>
          </form>
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-ink">
                Detail Request Edit {getClosingById(selectedReq.closing_id) ? `(${getClosingById(selectedReq.closing_id)!.date})` : ""}
              </h3>
